...
# -----------------------------
# This is passed to the OpenAI API so the model is constrained at
# generation time to only ever emit one of these literal values.
# This is the real fix: previously the model was free to emit any
# string for "task" (e.g. "compliance reasoning") because nothing
# enforced the enum on the API side - only Pydantic validated it
# *after* the fact, by which point it was already a failure.
